...
<%@ attribute name="addonly" type="java.lang.Boolean" required="false" description="只能新增,不能删除模式"%>
	...
	function ${input}Preview(){
		...
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">
				...
				li += "&nbsp;&nbsp;<c:if test="${!addonly&&!readonly}"><a href=\"javascript:\" onclick=\"${input}Del(this);\">×</a></c:if></li>";
