...
function FocusBtn(btn, workid) {
    ...
    $.ajax({ url: "Do.jsp?ActionType=Focus&WorkID=" + workid, async: false });
}
...
function initBar() {
    ...
    $.ajax({
        ...
        async: true,
        data: pageData,
        url: basePath+"/MyWF/myflow.do?Method=InitToolBar&m=" + Math.random(),
        dataType: 'html',
...
function setAttachDisabled() {
    //附件设置
    var attachs = $('iframe[src*="AttachmentUpload.jsp"]');
...
function Save() {
    ...
    $.ajax({
        ...
        url: "MyFlowModel.java?Method=Save",
        dataType: 'html',
        success: function (data) {
...
function initGroup(workNodeData, groupFiled) {
    ...
    switch (groupFiled.CtrlType) {
        ...
        case "Dtl":
            ...
            var src = "/WF/CCForm/Dtl.jsp?s=2&EnsName=" + groupFiled.CtrlID + "&RefPKVal=" + pageData.WorkID + "&PageIdx=1";
            src += "&r=q" + paras;
        ...
        case "Ath": //增加附件.
            ...
            for (var athIndex in workNodeData.Sys_FrmAttachment) {
                ...
                if (pageData.IsReadonly)
                    src = "/WF/CCForm/AttachmentUpload.jsp?PKVal=" + pageData.WorkID + "&Ath=" + ath.NoOfObj + "&FK_MapData=" + groupFiled.EnName + "&FK_FrmAttachment=" + ath.MyPK + "&IsReadonly=1";
                else
                    src = "/WF/CCForm/AttachmentUpload.jsp?PKVal=" + pageData.WorkID + "&Ath=" + ath.NoOfObj + "&FK_MapData=" + groupFiled.EnName + "&FK_FrmAttachment=" + ath.MyPK;
            ...
            break;
        case "FWC": //审核组件.
            var src = "/WF/WorkOpt/WorkCheck.jsp?s=2";
            ...
            break;
        case "SubFlow": //子流程..
            var src = "/WF/WorkOpt/SubFlow.jsp?s=2";
            var paras = pageParamToUrl();
            if (paras.indexOf('OID') < 0) {
                paras += "&OID=" + pageData.WorkID;
            }
        ...
        case "Track": //轨迹图.
            ...
            break;
        case "Thread": //子线程.
            var src = "/WF/WorkOpt/threaddtl.jsp?s=2";
            var paras = pageParamToUrl();
            if (paras.indexOf('OID') < 0) {
                paras += "&OID=" + pageData.WorkID;
            }
            ...
            break;
        case "FTC": //流转自定义.  有问题
            var src = "/WF/WorkOpt/FTC.jsp?s=2";
            var paras = pageParamToUrl();
            if (paras.indexOf('OID') < 0) {
                paras += "&OID=" + pageData.WorkID;
            }
...
function InitForm() {
    ...
    $('.divAth').bind('click', function (obj) {
        ...
        if (ath.length > 0) {
            ...
            var src = "";
            if (pageData.IsReadonly)
                src = "/WF/CCForm/AttachmentUpload.jsp?IsExtend=1&PKVal=" + pageData.WorkID + "&Ath=" + ath.NoOfObj + "&FK_MapData=" + groupFiled.FK_MapData + "&FK_FrmAttachment=" + ath.MyPK + "&IsReadonly=1";
            else
                src = "/WF/CCForm/AttachmentUpload.jsp?IsExtend=1&PKVal=" + pageData.WorkID + "&Ath=" + ath.NoOfObj + "&FK_MapData=" + groupFiled.FK_MapData + "&FK_FrmAttachment=" + ath.MyPK;
...
function refSubSubFlowIframe() {
    var iframe = $('iframe[src*="SubFlow.jsp"]');
    //iframe[0].contentWindow.location.reload();
    iframe[0].contentWindow.location.href = iframe[0].src;
}
//回填扩展字段的值
